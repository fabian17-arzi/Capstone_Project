...
export default function Arsip() {
  ...
  const [previewTitle, setPreviewTitle] = useState(""); // <-- TAMBAHAN
  ...
  const fetchDokumen = async (id) => {
    ...
    try {
      const resBAPB = await fetch(
        `http://localhost/capstone_backend/list_vendor_progres_bapb.php?vendor_id=${id}`
      );
      const resultBAPB = await resBAPB.json();

      const resBAPP = await fetch(
        `http://localhost/capstone_backend/list_vendor_progres_bapp.php?vendor_id=${id}`
      );
      ...
      if (resultBAPB.success && Array.isArray(resultBAPB.data)) {
        combinedData = [...combinedData, ...resultBAPB.data.map(item => ({ ...item, type: "BAPB" }))];
      }
      if (resultBAPP.success && Array.isArray(resultBAPP.data)) {
        combinedData = [...combinedData, ...resultBAPP.data.map(item => ({ ...item, type: "BAPP" }))];
      }
      ...
      archiveData.sort((a, b) => new Date(b.dibuat_pada) - new Date(a.dibuat_pada));
  ...
  const getStatusDisplay = (item) => {
    if (item.status === "Approved") {
      return (
        <p className="text-sm text-green-600 font-semibold">Status: Disetujui Penuh</p>
      );
    }
    ...
    <div className="p-6 w-full">
      <div className="w-full bg-gradient-to-r from-blue-700 to-blue-500 rounded-xl text-white p-6 shadow">
        ...
        <p className="text-sm opacity-80">
          Semua arsip dokumen Anda di sini ({dokumenList.length})
        </p>
      </div>
      ...
      <div className="mt-6 space-y-4">
        {dokumenList.length === 0 ? (
          <div className="bg-white p-4 rounded-md shadow text-gray-500">
            Tidak ada dokumen yang disetujui atau ditolak.
          </div>
        ) : (
